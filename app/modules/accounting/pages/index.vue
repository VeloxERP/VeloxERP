  ...
  <div class="container mx-auto py-6 dark:text-white">
    <div class="flex justify-between items-center mb-6">
      <div>
        ...
        <p class="text-muted-foreground">Financial overview and management</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <Card>
        ...
        <CardContent>
          <div class="text-2xl font-bold">€12,234.56</div>
          <p class="text-xs text-muted-foreground">
      ...
      <Card>
        ...
        <CardContent>
          <div class="text-2xl font-bold">€32,997.33</div>
          <p class="text-xs text-muted-foreground">
          ...
          <div class="space-y-4">
            <div v-for="transaction in recentTransactions" :key="transaction.id" class="flex items-center justify-between">
              <div class="flex items-center space-x-3">
                <div class="w-2 h-2 rounded-full" :class="transaction.type === 'income' ? 'bg-green-500' : 'bg-red-500'"></div>
                <div>
                  <p class="font-medium">{{ transaction.description }}</p>
                  <p class="text-sm text-muted-foreground">{{ transaction.account }}</p>
                </div>
              </div>
        ...
        <CardContent>
          <div class="space-y-4">
            <div v-for="account in accountBalances" :key="account.id" class="flex items-center justify-between">
              ...
              <div class="text-right">
                <p class="font-medium">€{{ account.balance }}</p>
...
import { DollarSign, Receipt, TrendingUp, Clock } from 'lucide-vue-next';
